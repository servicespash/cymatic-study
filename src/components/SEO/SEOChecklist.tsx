...
import { ShieldCheck, Award, CheckCircle, Info, RefreshCw, AlertTriangle, ExternalLink } from "lucide-react";
...
export function SEOChecklist() {
  ...
  const handleVerify = () => {
    if (!portfolioUrl.trim()) {
      toast.error("Portfolio Link Required", {
        description: "Please provide your portfolio or verification page link (e.g., https://cymatichub.xyz/latif)."
      });
      return;
    }
    ...
    if (reviewText.trim().length < 15) {
      toast.error("Detailed Review Required", {
        description: "Please provide a detailed positive review / quality affirmation of at least 15 characters."
      });
      return;
    }
    ...
    setTimeout(() => {
      ...
      const positiveWords = [
        "excellent", "great", "verified", "positive", "accurate", "reliable", "trustworthy",
        "approved", "correct", "love", "good", "perfect", "quality", "stellar", "recommend",
        "compliant", "certified", "expert", "best", "helpful", "ncdc", "uneb", "pedagogical"
      ];

      // Heuristic list of negative indicators
      const negativeWords = [
        "poor", "bad", "wrong", "plagiarized", "inaccurate", "fake", "reject", "unreliable",
        "broken", "terrible", "worst", "unverified", "outdated", "cheat"
      ];
      ...
      if (score >= 2 && negativeCount === 0) {
        setValidationResult({
          status: "APPROVED",
          score,
          feedback: "Dynamic E-E-A-T analysis passed successfully. High semantic quality, localized context, and positive sentiment signals are approved for publishing."
        });
        toast.success("E-E-A-T Validation Approved!", {
          description: "Your study notes and author credentials have been validated for immediate publication."
        });
      } else {
        const feedbackMessage = negativeCount > 0
  ...
  return (
    <div id="seo-eeat-checklist-card" className="rounded-3xl border border-border bg-card p-6 shadow-card space-y-6">
      ...
      <div className="rounded-2xl bg-primary/5 border border-primary/10 p-5 space-y-3">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          Google's Search Quality Evaluator Guidelines emphasize <strong>E-E-A-T (Experience, Expertise, Authoritativeness, and Trustworthiness)</strong>. This background system automates validation by scanning your portfolio links and assessing the peer reviews/affirmations submitted by content curators.
        </p>
        <p className="text-xs text-muted-foreground leading-relaxed">
          When you click <strong>Verify &amp; Authorize</strong>, our background engine analyzes the sentiment, checks for pedagogical alignment keywords (e.g., NCDC, UNEB, accurate, compliant), and ensures there are no conflicting trust flags. Passing this check embeds certified credentials in your platform’s metadata schemas to maximize AI engine citations.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Please provide a link to your verified academic portfolio. You can search or request credentials at{" "}
            <a
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Provide a clear, positive review verifying the syllabus accuracy, pedagogical quality, and completeness of the secondary curriculum lessons (Minimum 15 characters).
          </p>
          <textarea
            value={reviewText}
