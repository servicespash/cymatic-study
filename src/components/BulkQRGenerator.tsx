...
const DEFAULT_MOCK_SCHOOLS: OrganizationRecord[] = [
  ...
  { id: "SCH-NCDC-WAKISO", name: "St. Mary's College Kisubi (SMACK)", school_key: "NCDC-WAKISO-SMACK" },
...
export function BulkQRGenerator() {
  ...
  const filteredSchools = organizations.filter(org => 
    ...
    try {
      ...
      for (let i = 0; i < targets.length; i++) {
        ...
        if (cardIndexOnPage === 0) {
          ...
          doc.setFontSize(7);
          doc.text(`Official Registrar Distribution Copy  |  Page ${pageIndex + 1}`, 105, 284, { align: "center" });
        }
        ...
        doc.setFont("Helvetica", "bold");
        doc.setFontSize(8);
        doc.text("NATIONAL BOARDING INSTITUTION REGISTRY", xOffset + cardWidth / 2, yPos + 7, { align: "center" });

        doc.setFont("Helvetica", "bold");
        doc.setFontSize(10);
        const displayName = org.name.length > 36 ? org.name.substring(0, 34) + "..." : org.name;
        doc.text(displayName.toUpperCase(), xOffset + cardWidth / 2, yPos + 13, { align: "center" });
        ...
        doc.setFont("Helvetica", "bold");
        doc.setFontSize(7);
        doc.text("OFFICIAL QR BINDING KEY", xOffset + cardWidth / 2, yPos + 73, { align: "center" });

        doc.setFont("Helvetica", "normal");
        doc.setFontSize(6.5);
        doc.text("SCAN THIS QR CODE TO INSTANTLY LINK SCHOLARS & VERIFY RECORD COMPLIANCE", xOffset + cardWidth / 2, yPos + 80, { align: "center" });

        doc.setFont("Helvetica", "bold");
        doc.setTextColor(primaryColor);
        doc.text("MINISTRY OF EDUCATION & SPORTS UGANDA", xOffset + cardWidth / 2, yPos + 85, { align: "center" });
      }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <CardDescription className="text-zinc-400 text-xs mt-1">
              Select, brand, and compile print-ready sheet badges with QR links for multiple schools.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="hidden" aria-hidden="true">
          {organizations.map((org) => {
            const baseUrl = typeof window !== "undefined" ? window.location.origin : "https://study.cymatichub.xyz";
            ...
            return (
              <QRCodeCanvas
                key={`canvas-${org.id}`}
                ref={(el) => { canvasRefs.current[org.id] = el; }}
                value={qrPayload}
                size={256}
                level="H"
              ...
              {COLOR_PRESETS.map((p) => (
                <button
                  ...
                  className={`h-8 w-8 rounded-lg transition-all border-2 flex items-center justify-center ${p.bgClass} ${
                    primaryColor === p.value ? `${p.borderClass} scale-110 shadow-glow` : "border-transparent opacity-80 hover:opacity-100"
                  }`}
                  title={p.name}
                >
        ...
        <div className="border border-white/5 rounded-2xl overflow-hidden bg-zinc-950/40">
          <div className="flex items-center justify-between p-3 bg-white/5 border-b border-white/5 text-xs font-bold text-zinc-300">
            ...
            >
              {filteredSchools.length > 0 && filteredSchools.every(org => selectedIds.includes(org.id)) ? (
